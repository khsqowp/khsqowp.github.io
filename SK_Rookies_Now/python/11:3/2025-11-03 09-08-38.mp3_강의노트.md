...
오늘부터는 파이썬의 기본 문법을 넘어, 데이터 분석의 세계로 첫발을 내딛습니다. 파이썬이 데이터 과학 분야에서 강력한 언어로 자리 잡게 한 핵심 라이브러리들을 배우게 되는데, 그 첫 번째 주자가 바로 **NumPy**입니다.

강사님께서는 데이터 분석의 전체적인 흐름을 `NumPy` → `Pandas` → `Matplotlib`/`Seaborn` (시각화) → `Streamlit` (웹 대시보드) 순서로 진행할 예정이라고 하셨습니다. 이 흐름을 따라가기 위해, 오늘은 모든 데이터 분석의 기초가 되는 `NumPy` 배열의 개념과 사용법을 확실히 익히는 것을 목표로 합니다.

---

### 왜 파이썬 리스트 대신 NumPy 배열을 사용할까?

본격적인 학습에 앞서, "왜 이미 `list`라는 좋은 자료구조가 있는데 굳이 `NumPy`의 `array`를 배워야 할까?"라는 근본적인 질문을 던져볼 수 있습니다.
...
### 핵심 개념: 벡터화(Vectorization) 연산

**벡터화**는 `NumPy`를 사용하는 가장 큰 이유 중 하나입니다. 이는 반복문을 사용하지 않고 배열의 모든 요소에 대해 연산을 수행하는 기능입니다.
...
지금까지 배운 `NumPy` 개념을 종합하여, 서버 로그 데이터를 분석하고 그 결과를 가상의 AI 서비스에 연동하는 시나리오를 구현해 보겠습니다.
